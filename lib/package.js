...
/**
 * function package(dynamodb, models)
 * Returns an  dynamodb model representing
 * the metadata of a single version of a package
 * being persisted to the connection associated
 * with the `dynamodb` instance provided.
 *
 * @param {dynamodb} dynamodb Instance of dynamodb to attach to.
 * @param {Object} models Set of registry-data models.
 *
 * @returns {Object} an dynamodb model representing
 * the metadata of a single version of a package
 */
module.exports = function pkg(dynamodb, models) {
  const hashKey = 'name';
  const model = dynamodb.define('Package',{
    hashKey,
    tableName: 'WarehousePackage',
    schema: {
      name: Joi.string(),
      version: Joi.string(),
  ...
  var Pkg = new Dynastar({model, hashKey});
