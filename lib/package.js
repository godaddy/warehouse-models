...
module.exports = function pkg(dynamo) {
  ...
  ['one', 'all'].forEach(function (type) {
    model.after(['find', type].join(':'), function (result, callback) {
      callback(null, deserialize(result));
    });
  });
   ...
   * @returns {Object} ?
   */
  function fromPublish({ pkg, payload }) {
    var replica = assign({}, pkg.extractLatest(payload));
    var schemaFields = model.schema.mappedFields();
    var keys;
