 ...
 */
module.exports = function version(dynamodb) {
  const hashKey = 'name';
  const rangeKey = 'version';
  const model = dynamodb.define('Version', {
    hashKey,
    rangeKey,
  ...
  var Version = new Dynastar({model, hashKey, rangeKey});
   ...
   * @api public
   */
  Version.prototype.forBuild = function forBuild(read, done) {
    this.getAttachment(read, function gotAttach(err, pack) {
